...
def kernel(current, vol,vel,eff,mtype,a,b,c,a2,ratio,wtype,point1,point2,point3,point4,EnPulse,Ibase,Ubase,freq,alpha):
    if wtype == 'Line':
        T_Plain(current,vol,vel,eff,mtype,a,b,c,a2,ratio,point1,point2,point4,EnPulse,Ibase,Ubase,freq,alpha)

    elif wtype == 'Arc':
        Circle(current, vol,vel,eff,mtype,a,b,c,a2,ratio,point1,point2,point3,point4,EnPulse,Ibase,Ubase,freq,alpha)
...
def T_Plain(current,vol,vel,eff,mtype,a,b,c,a2,ratio,point1,point2,point3,
    ...
    pwd=os.getcwd()
    f.writelines("c=====================================================================\n\n")
    f.writelines("c        This subroutine is generated by WeldFlux v1.6.1            c\n")      
    ...
    if EnPulse:
        f.writelines("          freq = "+str(freq)+'\n')
        f.writelines("          alpha = "+str(alpha)+'\n')
        f.writelines("          baseI = "+str(Ibase)+'\n')
        f.writelines("          baseU = "+str(Ubase)+'\n')
        f.writelines("          Qmin = 1000.*baseI*baseU\n")
        f.writelines("          wpulse = 2*pi*freq\n")
        f.writelines("          t = time(1)\n")
        f.writelines("          CALL GETSERIES(alpha,wpulse,t)\n")
        f.writelines("          power = alpha*(power-Qmin)+2*(power-Qmin)*ft/pi+Qmin\n")
        f.writelines("          if (power<0.) power=0.\n")
    ...
    f.writelines("      real,parameter::pi=3.14159\n")
    f.writelines("      contains\n")
    f.writelines("      SUBROUTINE GETSERIES (alpha,wpulse,t)\n")
    f.writelines("        ft = 0.\n")
    f.writelines("        do n = 1,20,1\n")
    f.writelines("          ft = ft + sin(alpha*n*pi)*cos(n*wpulse*t)\n")
    f.writelines("        end do\n")
...
def Circle(current, vol,vel,eff,mtype,a,b,c,a2,ratio,point1,point2,point3,point4,EnPulse,Ibase,Ubase,freq,alpha):
    t1 = type(point1)
    t2 = type(point2)
    t3 = type(point3)
    t4 = type(point4)
